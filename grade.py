...
def grade(form, output_im, output_file):
    ...
    im3 = im2.rotate(angle_to_rotate)
    px3 = im3.load()
    ...
    for each in finalist:
        if each[7]-1 > last_number:
            if last_number != 0:
                if x_crossed == "x":
                    output_txt.write(new_line+" x"+"\n")
                else:
                    output_txt.write(new_line+"\n")
            while each[7]-1 > last_number:
                last_number += 1
                output_txt.write(str(last_number)+"\n")
            x_crossed = ""
            if each[4] == "x":
                x_crossed = "x"    
            new_line = str(each[7])+" "+each[3]
            last_number += 1
        elif each[7] > last_number and each[7]-1 == last_number:
            if last_number != 0:
                if x_crossed == "x":
                    output_txt.write(new_line+" x"+"\n")
                else:
    ...
    #Final Box output and marking X space
    for each in finalist:
        i,j = each[1:3]
        for x in range(36):
            for y in range(4):
